...
class FIRFilter:
    def __init__(self, filter='mean', nb_samples=10):
        self.nb_samples = nb_samples
        self.buffer = np.zeros([7, nb_samples])
        self.iterator = 0
...
class IIRLinearFilter:
    def __init__(self, order=5, wn=.075, fs=150):
        self.b, self.a = signal.butter(N=order, 
                                       Wn=wn, 
                                       #analog='False',
                                       output='ba',
                                       fs=fs)
        self.delay = np.zeros([7, order])

    def filter(self, measurement):
        z, self.delay = signal.lfilter(self.b, 
                                    self.a,
                                    x = measurement.reshape([7,1]), 
                                    zi=self.delay)
        return z.flatten()

class IIRQuadraticFitler:
    def __init__(self, order=5, wn=20, fs=150):
        self.sos = signal.butter(N=order, 
                                Wn=wn, 
                                #analog='False',
                                output='sos',
                                fs=fs)
        self.delay = np.zeros([self.sos.shape[0], 7, 2])
